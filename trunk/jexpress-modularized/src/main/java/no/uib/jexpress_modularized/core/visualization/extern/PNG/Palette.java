 ...
 * Representation of an indexed palette of arbitrary size. This can

 * be used by image encoders which require limited palettes (eg 256 colour PNGs).

 * The palette is autogenerated from a number of images and potentially some extra
 ...
 * totally transparent. (This means we can get away with a single index for

 * all transparent colours.) The generation method is pretty basic, removing a

 * single colour at a time. This is fine for images with a smallish number of

 * colours to begin with (eg tiled collections of 256-colour images) but bad

 * for full-colour photos.

 * <p>

 * Further documentation may be provided at some stage :)
...

{
    ...
    public Palette (BufferedImage[] images,
...

    {
         ...
         * Find the colour with the minimum population

         * Find the colour closest to it
...

        {
            ...
            closeCe.setRed ((closeCe.getRed()*closeCe.population+

                             minCe.getRed()*minCe.population)/

                            totalPop);

            closeCe.setGreen ((closeCe.getGreen()*closeCe.population+

                             minCe.getGreen()*minCe.population)/

                            totalPop);

            closeCe.setBlue ((closeCe.getBlue()*closeCe.population+

                             minCe.getBlue()*minCe.population)/
...

    {
         ...
         * Compare two colours. We ignore

         * the alpha value as transparency is just

         * the first colour, which is ignored in
